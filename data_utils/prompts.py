...
webact_online_sys_prompt = '''Imagine that you are imitating humans doing web navigation for a task step by step. At each stage, you can see the webpage like humans by a screenshot and know the previous actions before the current step decided by yourself through recorded history. You need to decide on the first following action to take. You can click on an element with the mouse, select an option, type text or press Enter with the keyboard. (For your understanding, they are like the click(), select_option() type() and keyboard.press('Enter') functions in playwright respectively) One next step means one operation within the four. Unlike humans, for typing (e.g., in text areas, text boxes) and selecting (e.g., from dropdown menus or <select> elements), you should try directly typing the input or selecting the choice, bypassing the need for an initial click. You should not attempt to create accounts, log in or do the final submission. Terminate when you deem the task complete or if it requires potentially harmful actions.'''

webact_online_question_description_new_exp4 = '''The screenshot below shows the webpage you see. Follow the following guidance to think step by step before outlining the next action step at the current stage:
...
webact_online_action_format = "ACTION: Choose an action from {CLICK, SELECT, TYPE, PRESS ENTER, TERMINATE, NONE}."

webact_online_value_format = "VALUE: Provide additional input based on ACTION.\n\nThe VALUE means:\nIf ACTION == TYPE, specify the " \
...
webact_choice_prompt_dict = {
    "system_prompt": webact_online_sys_prompt,

    "question_description": webact_online_question_description_new_exp4,
...
ELEMENT: The uppercase letter of your choice. (No need for PRESS ENTER)""",
    ...
    "action_format": f"{webact_online_action_format}",

    "value_format": f"{webact_online_value_format}"
}
...
def generate_prompt(experiment_split, task=None, previous=None, choices=None):
    ...
    if False:
        raise Exception("Please change experiment type to webact")
    elif experiment_split in ["webact_online","online","webact","webact"]:
        system_prompt_input = webact_choice_prompt_dict["system_prompt"]
        question_description_input = webact_choice_prompt_dict["question_description"]
        referring_input = webact_choice_prompt_dict["referring_description"]
        element_format_input = webact_choice_prompt_dict["element_format"]
        action_format_input = webact_choice_prompt_dict["action_format"]
        value_format_input = webact_choice_prompt_dict["value_format"]
        prompt_list = []
